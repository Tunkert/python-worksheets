        ...
        if self.numerator_product() >= self.denominator_product():
            for x in range(2, self.denominator_product()):
                if self.numerator_product() % x == 0 and self.denominator_product() % x == 0:
                    gcf = x
        if self.denominator_product() >= self.numerator_product():
            for y in range(2, self.numerator_product()):
                if self.numerator_product() % y == 0 and self.denominator_product() % y == 0:
                    gcf = y
        return gcf
